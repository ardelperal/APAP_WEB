"""LocalBackend adapter implementing :class:`AuthUsersPort` for ``usuarios_autorizados``.

The adapter is the only place in the auth slice that talks to the
:class:`~app.core.local_backend.db.LocalPostgresExecutor` (via the
:class:`~app.core.data_access.SqlExecutor` Protocol — the foundation
work's backend-agnostic contract). The SQL lives in
:mod:`app.core.local_backend.auth_queries` per AGENTS.md §22; this
module is pure orchestration: take the use-case input, run the SQL,
build the domain entity, propagate Protocol-level exceptions.
...
application lifespan's :class:`~app.core.local_backend.db.LocalPostgresExecutor`.
"""
...
from app.core.local_backend.auth_queries import (
...
# ``app.core.schema_bootstrap`` ↔ ``app.core.local_backend`` cycle:
...
class LocalBackendAuthUsersAdapter:
    """LocalBackend implementation of :class:`AuthUsersPort`.
    ...
    def ensure_schema_and_seed(self, initial_admin_email: str) -> None:
        ...
        # ``app.core.ports.__init__`` -> ``app.core.local_backend``.
    ...
    # --- Classic password auth stubs ---------------------------------
    # LocalBackend supports classic email/password login via
    # MagicLinkPort (magic links) and ClassicPasswordPort (Argon2id).
    # These stubs keep the AuthUsersPort contract complete: password
    # operations live on a separate port, not here.

    def verify_password(
        self, email: str, password: str
    ) -> AuthorizedUser | None:
        """LocalBackend delegates password verification to ClassicPasswordPort.
    ...
    def set_password(self, email: str, password: str) -> None:
        """LocalBackend delegates password management to ClassicPasswordPort.

        This stub raises so any accidental wiring of the wrong adapter
        fails loudly rather than silently doing nothing.
        """
        raise NotImplementedError(
            "LocalBackendAuthUsersAdapter cannot set_password; "
            "use ClassicPasswordPort instead."
        )

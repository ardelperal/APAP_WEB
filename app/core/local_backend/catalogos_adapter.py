...
column-name aliases, no schema-shaping, no transport quirks.

Rule §31 (domain depends on Protocol): the adapter constructor takes
a :class:`SqlExecutor`. The DI helper passes the
:class:`~app.core.local_backend.db.LocalPostgresExecutor` from the
...
# new tests under ``tests/test_insforge_catalogos_adapter.py`` pin the
...
def _coerce_bool(value: Any) -> bool:
    ...
    The SQL boolean may arrive as JSON bool (``True``/``False``), string
    (``"true"``/``"false"``), or integer (``1``/``0``). This helper is the
    ...
    def __init__(self, executor: SqlExecutor) -> None:
        ...
        Args:
            executor: Any object that satisfies the
                ...
                :class:`~app.core.local_backend.db.LocalPostgresExecutor`
                stored on ``app.state.sql_executor``; in tests it can be
                a ``httpx.MockTransport``-backed fake.
        """

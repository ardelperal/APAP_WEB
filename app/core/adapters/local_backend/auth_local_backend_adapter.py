...
LocalBackend transport (via the :class:`~app.core.data_access.SqlExecutor`
Protocol — the foundation work's backend-agnostic contract). The
SQL lives in :mod:`app.core.adapters.local_backend.auth_local_backend_queries`
...
already-pooled :class:`~app.core.local_backend.LocalPostgresExecutor` that the
...
# ``app.core.schema_bootstrap`` ↔ ``app.core.adapters.local_backend`` cycle:
...
class LocalBackendAuthUsersAdapter:
    ...
    def ensure_schema_and_seed(self, initial_admin_email: str) -> None:
        ...
        # ``app.core.ports.__init__`` -> ``app.core.adapters.local_backend.__init__``.
        # lazy-import: avoids the auth adapter/schema-bootstrap import cycle.
    ...
    # Classic password auth is not supported by LocalBackend. The
    # M1 local adapter (ClassicPasswordAuthPort) overrides
    # these; the LocalBackend adapter is the fallback when
    ...
    # LocalBackend is unsupported (raises).
    ...
    def set_password(self, email: str, password: str) -> None:
        """LocalBackend cannot store password hashes. Always raises.

        The login endpoint must not reach this path for LocalBackend
        deployments because the local adapter is wired in when
        APAP_LOCAL_BACKEND=true (and LocalBackend is not). The raise
        is a defensive guard: if a future code path wires the
        wrong adapter, the failure is loud.
        """
        raise NotImplementedError(
            "LocalBackend adapter cannot set_password; configure "

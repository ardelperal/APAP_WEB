"""InsForge adapter implementing :class:`CatalogosPort` for catalog reads.
...
InsForge envelope unwrapping is owned by :class:`LocalPostgresExecutor` in
``app/core/insforge.py``, and the adapter sees the canonical
...
def _coerce_bool(value: Any) -> bool:
    ...
    InsForge returns the boolean column as a JSON bool (``True``/``False``),
...
class InsForgeCatalogosAdapter(CatalogosPort):
    """Implements :class:`CatalogosPort` by querying InsForge via ``SqlExecutor``.

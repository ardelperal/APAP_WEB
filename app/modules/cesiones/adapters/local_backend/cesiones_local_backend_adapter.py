"""LocalBackend adapter — implements ``CesionesPort`` via the existing service.
...
class CesionesLocalBackendAdapter:
    """LocalBackend-backed adapter for ``CesionesPort``.

    Wraps the existing ``service.py`` use cases.
    The service uses ``SqlExecutor`` (no direct LocalBackend calls),

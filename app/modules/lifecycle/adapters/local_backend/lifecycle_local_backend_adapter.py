"""LocalBackend adapter implementing :class:`LifecyclePort`.

The adapter is the only module in the lifecycle slice that talks
to the LocalBackend transport (via the
:class:`~app.core.data_access.SqlExecutor` Protocol -- the
backend-agnostic contract introduced in issue #259). The SQL
lives in :mod:`app.modules.lifecycle.adapters.local_backend.lifecycle_local_backend_queries`
...
:class:`~app.core.local_backend.LocalPostgresExecutor` that the application
...
from app.modules.lifecycle.adapters.local_backend import lifecycle_local_backend_queries as q
...
class LocalBackendLifecycleAdapter:
    """LocalBackend implementation of :class:`LifecyclePort`.
...
__all__ = ["LocalBackendLifecycleAdapter"]

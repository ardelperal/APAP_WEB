"""Service layer for FOSTER-04 materiales catalog + junction (Refs #46).

Owns SQL, validation, mapping, soft-delete, and atomic cascade for the
``materiales`` table (catalog) AND the ``estancia_materiales`` junction
table that ties a material to a foster stay (estancia de acogida).
...
- ``remove_material_from_estancia`` flips a single junction row's
  ``activo = false``. Idempotent (returns False on missing /
  already-inactive).

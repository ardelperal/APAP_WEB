...
def _opt(value: str | None) -> str | None:
    ...
    Mirrors ``_opt`` in ``app/modules/materiales/routes.py`` and
    ``app/modules/foster/routes.py``. Used by ``_form_data_to_params``
    so ``notas`` collapses to NULL on a blank operator input instead
    of an empty string.
    """
    if value is None:
        return None

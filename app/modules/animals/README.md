...
The slice is fully converted to the hexagonal form described in §33 (epic #420). All nine port methods are landed (PRs #587, #596, #597, #603, #604, #609, #610, #611, #612). The legacy `service.py` still owns the legacy column surface (`TraeNChip`, `Raza`, etc.) until the dataclass is widened or a parallel `AnimalCreateRequest` lands — see the port's module docstring for the open question.

...
- :func:`actualizar_estado_animal`: re-derive the animal's current
  state via the lifecycle slice's domain cascade and upsert the
  cache row. Wired to :func:`app.modules.lifecycle.di.lifecycle_di.build_lifecycle_port`
  per AGENTS.md §33.4 — the simplified ``_EVENT_TYPE_TO_STATE`` map
  that lived here before LIFECYCLE-03 PR-C was the P1 fidelity gap
  (see REMOVED Requirement ``animal-state-simplified-direct-map`` at
  ``openspec/changes/lifecycle-state-resolver-33/specs/lifecycle/spec.md:248``).
...
def actualizar_estado_animal(
    ...
    *,
    ...
    """Re-derive and persist the animal's current state via the domain cascade.

    Called by :func:`record_event` after a lifecycle event is
    successfully inserted. The function builds the slice's
    :class:`~app.modules.lifecycle.ports.lifecycle_port.LifecyclePort`
    via :func:`app.modules.lifecycle.di.lifecycle_di.build_lifecycle_port`,
    asks it for the current ``DerivationResult`` (which runs the full
    P1-P6 priority cascade — see
    ``app/modules/lifecycle/domain/animal_state.py::calculate_state``),
    and persists the result to ``animal_current_state``.

    The previous implementation used a simplified
    ``_EVENT_TYPE_TO_STATE`` map that mapped the latest event type
    directly to a state without applying the cascade. That map was
    the P1 fidelity gap (see REMOVED Requirement
    ``animal-state-simplified-direct-map`` at
    ``openspec/changes/lifecycle-state-resolver-33/specs/lifecycle/spec.md:248``).
    LIFECYCLE-03 PR-C deletes the simplified map and wires the
    domain cascade via DI instead.

    Idempotent: the port's ``persist_animal_state`` uses
    ``ON CONFLICT (animal_id) DO UPDATE`` so re-running this for the
    same animal simply updates the cache row.
    """
    port = build_lifecycle_port(client)
    result = port.calculate_state(animal_id)

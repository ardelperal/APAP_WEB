...
| Capa de servicio | `app/core/auth.py` — `add_authorized_user` con pre-check + mapeo defensivo de `BackendError`; `get_user_by_email` con `normalize_email` |
...
1. **Fuente de verdad del problema**: #277 (AGENTS.md §32.P4) — `admin_add_user` capturaba `ValueError` pero dejaba escapar `BackendError` (p. ej. una violación de UNIQUE residual entre el pre-check y el INSERT) hacia un 500 no manejado. No existía handler global de excepciones en `app/` que la recogiera. #278 (AGENTS.md §32.P1, instancia nombrada) — el email nunca se normalizaba en el borde. `usuarios_autorizados.email` es `TEXT UNIQUE` con collation sensible a mayúsculas, así que `Maria.Lopez@Example.COM` y `maria.lopez@example.com` eran dos usuarios distintos. La capa de caché heredaba el problema porque la clave era el string exacto pasado.
2. **TDD estricto**: safety net inicial con 2546 passed antes del primer commit. RED (commit `5de64b7`) con 5 failed, 2541 passed, cubriendo los cinco átomos: normalización, validación de formato, duplicado vía pre-check, duplicado vía `BackendError`, e invalidación de caché por variante de case. GREEN (commit `f41e717`) con todos los átomos en verde; 2568 passed netos. Refactor mínimo para mantener el tamaño de `app/main.py` por debajo del presupuesto (AGENTS.md §21): los helpers de flash se extrajeron a `app/core/admin_helpers.py`.
...
|---|---|---|---|
| CRITICAL | `admin_add_user` no manejaba `BackendError` cuando el INSERT caía con violación de UNIQUE residual | fixed | El 500 escapaba sin contexto (§32.P4 — partial exception handling). Mitigación: pre-check explícito (`_CHECK_DUPLICATE_EMAIL_SQL`) + `except BackendError` defensivo que re-mapea a `ValueError("email already authorized: <canonical>")`; helper `_add_user_or_error` que captura `ValueError` y lo renderiza como flash. Átomos: `test_add_authorized_user_rejects_duplicate_via_backend_error`, `test_admin_add_user_with_duplicate_email_shows_error`. |
...
- **Defense in depth**: el pre-check `_CHECK_DUPLICATE_EMAIL_SQL` es la vía rápida y testeable; el `except BackendError` defensivo cubre el residuo entre el SELECT y el INSERT (carrera TOCTOU imposible por la UNIQUE constraint). Ambas vías terminan en el mismo `ValueError` que el route renderiza.

...
- `APAP_LOCAL_BACKEND_SERVICE_KEY` está disponible sólo en el shell del operador, nunca en el repositorio.
...
- Si el comando devuelve `bucket_public_violation`, deténgase. No aplique datos. Recree el bucket como privado mediante la herramienta de infraestructura de LocalBackend y reejecute el checkpoint de sólo lectura.
- Si el comando devuelve `bucket_visibility_unknown`, deténgase. Use la herramienta MCP `list-buckets` de LocalBackend para verificar la visibilidad antes de continuar.
...
- **Cuarentena del bucket en lugar de `delete-bucket`:** mantenga los objetos del bucket en su sitio y evite nuevas subidas eliminando el permiso del rol de servicio de LocalBackend para el bucket. Las fotografías existentes permanecen disponibles para lecturas autenticadas; el rastro de auditoría permanece intacto.
...
- **Precondiciones (deben cumplirse antes de que el operador ejecute el comando):**
    1. Una copia de seguridad verificada de la tabla (por ejemplo, `pg_dump --table web_only_feature_shadow`) se almacena fuera de la base de datos LocalBackend afectada y la ruta queda registrada en el ticket del operador.
...
- **Precondiciones (deben cumplirse antes de que el operador ejecute el comando):**
    1. Una exportación verificada del contenido del bucket (descarga desde LocalBackend Storage o equivalente) se almacena fuera del despliegue LocalBackend afectado y la ruta queda registrada en el ticket del operador.
...
Revierta el commit de PR2. Esta operación elimina el checkpoint del CLI, el cableado del bootstrap del bucket y las pruebas, sin tocar porciones de migración no relacionadas ni estado vivo de LocalBackend.

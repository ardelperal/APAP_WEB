...
>
> **Cerradas hoy (2026-07-27) — issue #32 LIFECYCLE-02 schema del timeline:**
>
> - **#32** `feat(animals): schema append-only animal_lifecycle_events + cache materializado` — PR <TBD>; tabla `animal_lifecycle_events` con 12 columnas + CHECK constraint sobre 14 event types (10 core + 4 supporting per `docs/legacy-lifecycle-transition-rules.md`) + 2 índices query-shape (`idx_animal_lifecycle_events_animal_timestamp` sobre `(animal_id, event_timestamp DESC)` + `idx_animal_lifecycle_events_caused_by` sobre `caused_by_event_id`); tabla `animal_current_state` con 11 columnas + 1 índice query-shape (`idx_animal_current_state_state` sobre `current_state`); enforcement append-only en SQL vía trigger `animal_lifecycle_events_append_only` (BEFORE UPDATE OR DELETE → `raise_append_only_violation()` plpgsql function que RAISE EXCEPTION, instalado en orden `CREATE FUNCTION → DROP TRIGGER IF EXISTS → CREATE TRIGGER` para idempotencia); causal pair rule D-23 (`FOSTER_CLOSED_BY_ADOPTION` precede `ADOPTION_STARTED` para el mismo animal) enforced en service `app/modules/animals/lifecycle_events.py` con `LifecycleEventType` StrEnum (10 core + 4 supporting) + `CausalPairViolation(D-23)` typed exception + `record_event` (INSERT con `ON CONFLICT (animal_id, event_type, event_timestamp) DO NOTHING` para idempotencia) + `validate_causal_pair` (pre-flight SELECT scoped a `(animal_id, FOSTER_CLOSED_BY_ADOPTION|ADOPTION_STARTED)` que raises `CausalPairViolation` cuando el par está fuera de orden); módulo exportado vía `app.modules.animals.__init__` per AGENTS §27 (consumers: futura PR de #33 state resolver + ya consumible por `migration/post_apply_diff` hook); migracion idempotente (todos los statements usan `IF NOT EXISTS` o `DROP IF EXISTS`); 16 átomos service en `tests/test_lifecycle_events.py` (enum contract + validation + INSERT shape + ON CONFLICT + D-23 OK/violation paths + SELECT scoping) + 9 átomos schema en `tests/test_domain.py` (4 índices + trigger + idempotencia + count statement 19→25); cobertura 93.51% (sobre floor 80%), `CRITICAL_HELPERS` 23/23 @100% PASS; sin audit dedicado (feature estructural sin superficie de seguridad nueva — el append-only trigger es la garantía). Bloquea LIFECYCLE-03 (#33 state resolver) y desbloquea el consumer de `migration/semantic_events.py::_persist_lifecycle_event` (issue #93 / `live-data-migration-sandbox`); el trigger function se documenta en `app/core/domain_lifecycle.py` docstring para que el siguiente lector entienda que cualquier futuro ALTER que lo dropee debe venir con la misma justificación que tomó añadirlo.
>

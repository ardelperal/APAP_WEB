...
**Última actualización:** 2026-07-27 — refresco de sincronización: 22 cierres del batch audit anti-pattern 2026-07-25 + hardening reflejados en §4 (#216 + #210 + #275-#294); nuevo bloque «Cerradas hoy (2026-07-25/27)» añadido antes del bloque 2026-07-24 (#216 y #210 no quedaron reflejadas en refrescos anteriores, ambas entran en este catch-up); filas de #216 y #210 retiradas de la tabla §4. Sin cambios de fase en §3 — todos los cierres son hardenings tácticos (security headers, rate limiting, JsonFormatter redaction, auth hardening, deploy step live, audit anti-patterns §32) y refactors de higiene (domain split, gate tooling), no transiciones de fase. §10 no se toca (describe intención original de los cohortes; #210 ya ejecutada). §11 aplicado: cada cierre dejó su fila con PR + SHA merge + una línea de lo entregado (audit doc referenciado cuando existe). Cierres adicionales del batch de hoy: **#205** refactor query builders trilogy (PRs #274/#296/#318), **#218** conn.commit() silent durability (PR #319), **#32** LIFECYCLE-02 schema append-only (PR #320), **#36** VOL-03 dedup fuzzy (PR #321, este merge) — todos reflejados en §4 con PR + SHA + una línea cada uno.
...
## 4. Issues abiertos (refresco 2026-07-27)
...
> **Cerradas hoy (2026-07-25 → 2026-07-27) — 22 cierres del batch audit anti-pattern 2026-07-25 + hardening:**
>
> Nota: este bloque agrupa los 22 cierres del ciclo de catch-up desde el refresco 2026-07-22. Incluye **#210** (cerrada 2026-07-22 vía PR #270 `43f86c2`) y **#216** (cerrada 2026-07-25 vía PR #297 `b87a0f9`), que tampoco habían quedado reflejadas en refrescos anteriores — entran aquí como catch-up. Los 20 restantes (#275-#294) se cerraron 2026-07-26/27, derivados del audit full-codebase del 2026-07-25 (#294, etiqueta `audit-2026-07-25`). Cada bullet lleva PR + merge SHA + una línea de lo entregado; audit doc referenciado cuando existe.
>
> - **#210** `fix(make) check-rules` — merged via PR #270 (`43f86c2`); APAP001/APAP003 linter ahora escanea la raíz del repo (`scripts/check_rules.py .`), destapando los detectors 5-8 (`print_in_app`, `csrf_middleware_registered`, `csrf_samesite_strict`, `duplicate_helper_definition`, `unjustified_lazy_import`, `cross_module_submodule_import`, `cross_module_private_import`).
> - **#216** `migration/dysflow_client.py → migration/legacy_access_client.py rename` — merged via PR #297 (`b87a0f9`); alinea el nombre del adaptador al seam `legacy_access_client` que ya consumía `app/core/...` desde la §18.1, eliminando la trampa cognitiva del nombre "dysflow".
> - **#275** `fix(config) startup fails when APAP_SESSION_SECRET missing/dev placeholder` — merged via PR #307 (`d145411`); el lifespan refuse to serve con el placeholder dev (cierre `§32.P2`); audit [`docs/audits/secret-startup-validation-2026-Q3.md`](audits/secret-startup-validation-2026-Q3.md) **PASS**.
> - **#276** `feat(security) HTTP security headers middleware` — merged via PR #311 (`9a3c63d`); CSP, X-Frame-Options, HSTS, nosniff + Referrer-Policy en cada respuesta (cierre `§32.P1`); audit [`docs/audits/security-headers-2026-Q3.md`](audits/security-headers-2026-Q3.md) **PASS**.
> - **#277** `fix(admin) /admin/users 500 on duplicate email` — merged via PR #308 (`b204a16`); render de flash user-facing en lugar de dejar `InsForgeError` sin manejar (cierre `§32.P4`).
> - **#278** `fix(auth) email never normalised` — merged via PR #308 (`b204a16`); email normalizado en el perímetro (boundary), dedup keys estables, cache invalidation precisa (cierre `§32.P1`); audit [`docs/audits/ghost-users-audit-2026-Q3.md`](audits/ghost-users-audit-2026-Q3.md) **PASS**.
> - **#279** `fix(auth) last-developer lockout when last active developer is deactivated` — merged via PR #309 (`0e8148d`); atomic guard en servicio + SEED filter en admin + flash message bloquean el orphaned-admin state (no se puede dejar el sistema sin developer activo).
> - **#280** `fix(auth-cache) invalidate_all() resets generations` — merged via PR #310 (`4d2b8c8`); `invalidate_all()` ya no resetea las per-email generations, eliminando la reapertura de la race write-after-invalidate; audit [`docs/audits/auth-cache-shared-2026-Q3.md`](audits/auth-cache-shared-2026-Q3.md) §"Issue #280 close (2026-07-27)" ampliado.
> - **#281** `fix(ci) deploy job never runs (merge-commit guard contradicts PR-only)` — merged via PR #302 (`0fa758c`); merge-commit skip eliminado, deploy step ya se ejecuta (cierre `§32.P7` — guard que se cancelaba a sí mismo).
> - **#282** `ci(test) provision PostgreSQL for TOCTOU test` — merged via PR #316 (`4b7af23`); `tests/test_voluntarios_concurrent.py` ya no es `--deselect`-ed en CI — el test corre de verdad bajo `services.postgres` (cierre `§32.P6`).
> - **#283** `fix(logging) log_safe KeyError on reserved LogRecord field names` — merged via PR #301 (`92fba88`); nested `_caller_fields` envelope con dotted lookup, evita pisar `LogRecord.args`/`name`/`msg`/`levelname`.
> - **#284** `fix(logging) JsonFormatter emits ~15 internal LogRecord fields` — merged via PR #301 (`92fba88`); allow-list estricto en `JsonFormatter` evita la fuga de internals de `asyncio`/`process` (`created`, `msecs`, `relativeCreated`, `process`, `thread`, `threadName`, `taskName`, `exc_text`, ...); audit [`docs/audits/logging-audit-2026-Q3.md`](audits/logging-audit-2026-Q3.md) **PASS**.
> - **#285** `fix(animals) animal_foto buffering/cache/docstring` — merged via PR #304 (`c969433`); streaming real con `ETag` + `Cache-Control: private, max-age=...` + module docstring refrescado (cierre `§32.P5`).
> - **#286** `feat(security) rate limiting on OAuth flow + write routes` — merged via PR #306 (`2a79929`); `SlowAPI` middleware gates `/auth/google`, `/auth/callback`, `/auth/post_logout` y todas las write routes (POST/PUT/DELETE/PATCH). (cierre `§32.P1`).
> - **#287** `refactor(auth-cache) resolve RedisAuthCache dead seam` — merged via PR #298 (`4b18ca4`); Redis selector eliminado, `APAP_AUTH_CACHE_BACKEND` validado `in_process`-only al startup, `redis` y valores desconocidos fallan `Settings` (cierre `§32.P3` + alinea con `§29`); audit [`docs/audits/auth-cache-in-process-audit-2026-Q3.md`](audits/auth-cache-in-process-audit-2026-Q3.md) **PASS**.
> - **#288** `test(routes) close route-layer coverage gap` — merged via PR #315 (`fabf964`); añade tests por-capa de routes para que la distribución del coverage sea visible (el promedio global 89% ocultaba capas entre 57% y 83%) (cierre `§32.P8`).
> - **#289** `refactor(core) split domain.py and acogidas/service.py` — merged via PR #303 (`517e500`); `app/core/domain.py` split en 9 cohesive modules (estaba a 1-2 líneas del budget 700 §21); `acogidas/service.py` ya estaba bien.
> - **#290** `feat(tooling) gate for AGENTS rule 22 (query-builder seam)` — merged via PR #305 (`5c39adf`); Detector 13 enforced en `scripts/check_rules.py` (cierre `§32.P3`); `materiales/queries.py` y `acogidas/queries.py` ya adoptan el pattern.
> - **#291** `docs(agents) AGENTS.md has two rules numbered 29` — merged via PR #295 (`660a01e`); colisión de numeración resuelta renombrando la primera §29 (auth cache scope) a la siguiente regla libre, en el mismo PR que #293.
> - **#292** `test(tooling) test_coverage_gate subprocess depends on ambient sys.path` — merged via PR #313 (`1790324`); `tests/test_coverage_gate.py` usa pytester con `--import-mode=importlib` y `PYTHONPATH` explícito, sin depender de la `sys.path` ambiente del host (cierre `§32.P6`).
> - **#293** `docs(agents) encode 2026-07-25 audit anti-patterns as AGENTS.md rule` — merged via PR #295 (`660a01e`); añade `§32.P1-§32.P8` con criterios numerados anclados a las 8 instancias del audit #294, cada uno con instance reference y review-criterion concreto (cierre `§32.P3` para `AGENTS.md` mismo).
> - **#294** `epic 2026-07-25 full-codebase audit — findings index` — merged via PR #312 (`04ebce2`); epic cerrado con findings labelled `audit-2026-07-25`; el checklist vivo queda en [`docs/audits/2026-07-25-audit-checklist.md`](audits/2026-07-25-audit-checklist.md); los 19 issues abiertos derivados del audit (#275-#293) ya están todos merged.
> - **#205** refactor query builders (issue trilogy — materiales PR #274 `7649948` 2026-07-25 + acogidas PR #296 `f4ce8ef` 2026-07-25 + adopciones PR #318 este merge 2026-07-27) — cierra el issue §205 con la adopción del query-builder seam (AGENTS §22) en los tres módulos principales; `adopciones/service.py` 654L → 272L; `app/modules/adopciones/queries.py` (nuevo, 198L) con 9 builders (`insert`, `list`, `get_by_id`, `update`, `delete`, `search`, `check_animal`, `check_voluntario`, `check_entrada`); 14 átomos golden en `tests/test_adopciones_queries.py` + 38 átomos existentes pasan byte-identically. Cero cambio de comportamiento observable; cobertura 93.37% (sobre floor 80%); `CRITICAL_HELPERS` 23/23 @100%.
> **Cerradas hoy (2026-07-24) — issue #204 refactor de `app/main.py`:**
>
> - **#204** `refactor(app): extraer middleware de auth y registro de rutas de app/main.py` — PR #271 ([squash `c92975b`+`28a5e3f`](https://github.com/ardelperal/APAP_WEB/pull/271), merge `2f93a95`, +1046/-168 sobre 9 archivos, `size:exception` pre-MVP autorizado por mantenedor "no reviewers in pipeline"); entrega `app/core/middleware.py::install_auth_middleware` (197L) + `app/routes_registry.py::register_routers` (62L) + `app/main.py` 689→544 líneas (~21% reducción); cadena middleware preservada (`UADetection → protect_user_facing_routes → Csrf`); cobertura 89.18% (sobre floor 80%), `CRITICAL_HELPERS` 21/21 @100% PASS; audit [`docs/audits/issue-204-middleware-extraction-audit-2026-Q3.md`](audits/issue-204-middleware-extraction-audit-2026-Q3.md) **PASS**; CI run `29953269866` `conclusion=success` (lint + test + build; e2e/deploy SKIPPED — esperado pre-MVP); 4R review (`review-97d8a8b3ffb21f14`) con 0 P0/P1 + 6 follow-ups WARNING-tier documentados (ver «Follow-ups from PR #271» abajo); rama `refactor/issue-204-auth-extraction` borrada en local y en origin per AGENTS §15.2
>
> **Cerradas hoy (2026-07-27) — issues #32 LIFECYCLE-02 + #36 VOL-03:**
>
> - **#32** `feat(animals): schema append-only animal_lifecycle_events + cache materializado` — PR #320 (merge `f041275`); tabla `animal_lifecycle_events` con 12 columnas + CHECK constraint sobre 14 event types (10 core + 4 supporting per `docs/legacy-lifecycle-transition-rules.md`) + 2 índices query-shape (`idx_animal_lifecycle_events_animal_timestamp` sobre `(animal_id, event_timestamp DESC)` + `idx_animal_lifecycle_events_caused_by` sobre `caused_by_event_id`); tabla `animal_current_state` con 11 columnas + 1 índice query-shape (`idx_animal_current_state_state` sobre `current_state`); enforcement append-only en SQL vía trigger `animal_lifecycle_events_append_only` (BEFORE UPDATE OR DELETE → `raise_append_only_violation()` plpgsql function que RAISE EXCEPTION, instalado en orden `CREATE FUNCTION → DROP TRIGGER IF EXISTS → CREATE TRIGGER` para idempotencia); causal pair rule D-23 (`FOSTER_CLOSED_BY_ADOPTION` precede `ADOPTION_STARTED` para el mismo animal) enforced en service `app/modules/animals/lifecycle_events.py` con `LifecycleEventType` StrEnum (10 core + 4 supporting) + `CausalPairViolation(D-23)` typed exception + `record_event` (INSERT con `ON CONFLICT (animal_id, event_type, event_timestamp) DO NOTHING` para idempotencia) + `validate_causal_pair` (pre-flight SELECT scoped a `(animal_id, FOSTER_CLOSED_BY_ADOPTION|ADOPTION_STARTED)` que raises `CausalPairViolation` cuando el par está fuera de orden); módulo exportado vía `app.modules.animals.__init__` per AGENTS §27; migracion idempotente (todos los statements usan `IF NOT EXISTS` o `DROP IF EXISTS`); 16 átomos service en `tests/test_lifecycle_events.py` (enum contract + validation + INSERT shape + ON CONFLICT + D-23 OK/violation paths + SELECT scoping) + 9 átomos schema en `tests/test_domain.py` (4 índices + trigger + idempotencia + count statement 19→25); cobertura 93.51% (sobre floor 80%), `CRITICAL_HELPERS` 23/23 @100% PASS; bloquea LIFECYCLE-03 (#33 state resolver).
> - **#36** `VOL-03: pipeline de deduplicación fuzzy de voluntarios legacy` — PR #321 (merge `<este PR>`, 2026-07-27); entrega `migration/volunteer_dedup.py` (puro: `VolunteerRef` + `MergedCluster` + `dedup_volunteers` con union-find + 3 pre-passes exact-name / DNI / fuzzy + detección de ambigüedad DNI + idempotencia por construcción) + `migration/cli_volunteer_dedup.py` (bridge CLI: `--input` / `--output` / `--summary`, JSON output como review surface per issue spec, exit codes 0/2/5); 19 átomos TDD en `tests/migration/test_volunteer_dedup.py` (11) + `tests/migration/test_cli_volunteer_dedup.py` (8) cubriendo fuzzy match across 3+ tablas, DNI como secondary key, ambigüedad flagged para revisión, idempotencia (doble ejecución byte-equal), invariante de PII redaction vía `caplog`; decisión **D-25** documentada en [`docs/decisiones-proyecto.md`](decisiones-proyecto.md) §3 sobre librería `rapidfuzz>=3.0` (no `thefuzz` — abandonado 2024-01, AGENTS §8); nueva dep `rapidfuzz>=3.0` agregada a `[project.optional-dependencies.etl]` en `pyproject.toml`.
>
> **Follow-ups from PR #271 (review 4R `review-97d8a8b3ffb21f14`, 2026-07-24) — WARNING-tier, 0 P0/P1:**
>
> El review 4R de PR #271 (`review-97d8a8b3ffb21f14`) cerró en **PASS** con 0 P0/P1 y 6 hallazgos WARNING-tier que **no bloquean el cierre de #204** pero se documentan como deuda técnica trackeada. **No se abren issues separadas en este refresh** — los detalles viven en `## Findings` + `## Correction (2026-07-22)` del audit [`docs/audits/issue-204-middleware-extraction-audit-2026-Q3.md`](audits/issue-204-middleware-extraction-audit-2026-Q3.md). Resumen operativo:
>
> 1. **Detector 7 (`csrf_middleware_registered`) unreachable en `app/core/middleware.py`**: el audit claim dice que Detector 7 ahora escanea dos archivos (`app/main.py` + `app/core/middleware.py`), pero `scripts/check_rules.py:479` solo invoca `_check_csrf_middleware_registered` cuando `_is_app_main(path, repo_root)` devuelve true. El bloque de scan de `app/core/middleware.py` (líneas 877-894 dentro de `find_violations`) **nunca se alcanza** vía el recorrido normal. **Fix**: cablear Detector 7 para que también escanee `app/core/middleware.py` directamente, no condicional a `_is_app_main`.
> 2. **`_redirect` helper duplicado byte-idéntico**: aparece copiado en `app/main.py:205-206` y `app/core/middleware.py:161-163`. Por AGENTS §25 (y §4 single source of truth) hay que consolidarlo en `app/core/responses.py` (o similar). **Fix en un PR de follow-up separado**; #204 ya está cerrada.
> 3. **`test_register_routers_does_not_duplicate_routes` es una no-op assertion**: camina dos instancias FastAPI distintas con el registry y compara sets, así que un futuro bug donde el registry doble-incluya un router no se cazaría. **Fix**: reescribir el test para llamar `register_routers` dos veces sobre la **misma** app y assertar que la superficie de rutas no cambia.
> 4. **Walker depende de atributo privado de FastAPI**: `tests/test_routes_registry.py` confía en `_IncludedRouter.original_router` (atributo privado). Si FastAPI ≥0.138 reorganiza el wrapper, el walker silenciosamente descarta las mismas rutas en ambas apps y el test pasa vacuo. **Fix**: pinear `fastapi<0.138` upper bound en `pyproject.toml` hasta que FastAPI exponga una API pública de route-introspection.
> 5. **`install_auth_middleware` no emite evento de startup observability**: considerar emitir `log_safe('middleware.installed', chain=[...], csrf_enabled=...)` al final de install, para que el panel de control pueda confirmar en runtime que la cadena middleware está activa (cierre de la columna «Panel de control / configuración» de §3 transversal Traza canónica).
> 6. **`install_auth_middleware(app, settings)` recibe `settings: object` (untyped)**: workaround documentado de un ciclo de Detector 11 (lazy-import). **Considerar añadir un contract test defensivo** que ancle el shape esperado de `settings` (al menos los atributos `csrf_enabled`, `session_cookie_name`).
...
| #218 | `execute_legacy_write`: `conn.commit()` failure swallowed (silent durability gap) | Migración | ✅ cerrada 2026-07-27 (PR #319, fix/issue-218-commit-durability) — ver §"Cerradas hoy (2026-07-27)" abajo |
...
**Cerradas hoy (2026-07-27):**

- **#218** `execute_legacy_write: conn.commit() failure swallowed (silent durability gap)` — PR #319 (merge `1bcc5b2`, este PR); fix P1 silent-durability-gap en `execute_legacy_write`: la función hacía `except pyodbc_mod.Error: pass` sobre `conn.commit()`, devolviendo rowcount como éxito cuando el commit había fallado y la escritura quedó rolled back. Nuevo typed `LegacyWriteCommitFailed(LegacyReaderError)` (definido en `migration/legacy_reader.py` junto a su padre, re-exportado por `migration/legacy_access_client.py` vía `__getattr__` PEP 562 + `TYPE_CHECKING` para resolver el ciclo); `execute_legacy_write` re-raises `pyodbc.Error` desde `conn.commit()` como `LegacyWriteCommitFailed` con `__cause__` encadenado al original. El per-row guard del orchestrator (`migration/reverse_apply/orchestrator.py`) añade `except LegacyWriteCommitFailed: raise` *antes* del catch-all `Exception` para que la failure se propague como categorical al CLI handler (`migration/cli_apply_reverse.py`), que la captura vía `except LegacyReaderError` y exits 5 con `legacy_read_failed` (extended en `docs/runbooks/live-migration-apply.md` para mencionar commit failure). El `log_safe("sync.applied", ...)` per-row queda automáticamente gated: la audit log se emite *después* del write seam, así que un `LegacyWriteCommitFailed` interrumpe la fila antes del log call. TDD: 7 átomos en `tests/migration/test_legacy_write_commit.py` (subclass / raises on commit failure / closes connection on commit failure / no rowcount-swallow / happy path regression / `sync.applied` no emitido / propagación al caller de `apply_web_to_legacy`). coverage 93.45% (≥80% floor), ruff + mypy + check_rules.py + check_module_size.py green.
- **#51** `HEALTH-02: API batch de actuaciones con commit transaccional + staging preview + D-24 per-record` — PR #322 (merge `<este PR>`); endpoint `POST /sanidad/actuaciones/batch` acepta array de N>=5 `actuacion_sanitaria` records. Atomicidad garantizada por CTE PostgreSQL con `unnest()` + JOIN contra `animales`/`voluntarios`/`catalogos_pruebas` + `bool_and(is_valid)` que gatea la `INSERT ... WHERE $8::boolean = false` (staging preview = `$8=true`, sin INSERT); el statement evalúa todo bajo un único snapshot, así que cualquier fallo invalida el batch completo (rollback total) — un half-batch es estructuralmente imposible. Validación D-24 per-record (reglas 1+2 vía `sanidad.service._validate_fecha_d24` antes del CTE; regla 3 vía JOIN a `animales.fecha_alta` dentro del snapshot). `dry_run=true` da staging preview sin INSERT (devuelve `BatchPreview(ok_count, error_count, items)` con `BatchItem(index, status, reason)` por record). `dry_run=false` y todos válidos → `INSERT` + `log_safe("sanidad.batch_committed")` + redirect 303 a `/sanidad`. Cualquier fallo → `BatchValidationError(failed_indices, reasons)` con disambiguation que preserva el vocabulario del single-record (`animal_no_activo`/`fecha_anterior_alta`/`voluntario_inactivo`/`tipo_prueba_inexistente`). Reutiliza `create_actuacion_sanitaria._validate_fecha_d24` y `_build_write_params` para no duplicar lógica (regla §25). Archivos: `app/modules/sanidad/queries.py` (CTE builder, AGENTS §22 seam) + `app/modules/sanidad/batch_service.py` (orquestación) + `app/modules/sanidad/batch_routes.py` (handler ≤50L, regla §28) + `app/templates/sanidad/batch_preview.html`. Tests TDD strict: 9 atoms builder en `tests/test_sanidad_queries.py` + 16 atoms service en `tests/test_sanidad_batch.py` (atomic commit, per-record D-24, staging preview, FK, idempotence al nivel de fila) + 8 atoms routes adicionales en `tests/test_sanidad_routes.py` (auth, 422/422/422/422 paths, reader rol, no-SQL-en-routes) + `sanidad/batch_preview.html` agregada a `test_xss_audit.py::TEMPLATE_SPECS`.

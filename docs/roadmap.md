...
- #218 (`eb44c96` PR #319 `fix/issue-218-commit-durability`) — fix P1 silent-durability-gap en `execute_legacy_write`: la función hacía `except pyodbc_mod.Error: pass` sobre `conn.commit()`, devolviendo rowcount como éxito cuando el commit había fallado y la escritura quedó rolled back. Nuevo typed `LegacyWriteCommitFailed(LegacyReaderError)` (definido en `migration/legacy_reader.py` junto a su padre, re-exportado por `migration/legacy_access_client.py` vía `__getattr__` PEP 562 + `TYPE_CHECKING` para resolver el ciclo); `execute_legacy_write` re-raises `pyodbc.Error` desde `conn.commit()` como `LegacyWriteCommitFailed` con `__cause__` encadenado al original. El per-row guard del orchestrator (`migration/reverse_apply/orchestrator.py`) añade `except LegacyWriteCommitFailed: raise` *antes* del catch-all `Exception` para que la failure se propague como categorical al CLI handler (`migration/cli_apply_reverse.py`), que la captura vía `except LegacyReaderError` y exits 5 con `legacy_read_failed` (extended en `docs/runbooks/live-migration-apply.md` para mencionar commit failure). El `log_safe("sync.applied", ...)` per-row queda automáticamente gated: la audit log se emite *después* del write seam, así que un `LegacyWriteCommitFailed` interrumpe la fila antes del log call. TDD: 7 átomos en `tests/migration/test_legacy_write_commit.py` (subclass / raises on commit failure / closes connection on commit failure / no rowcount-swallow / happy path regression / `sync.applied` no emitido / propagación al caller de `apply_web_to_legacy`). coverage 93.45% (≥80% floor), ruff + mypy + check_rules.py + check_module_size.py green. PR URL pendiente — añadir en commit amend tras `gh pr create`.

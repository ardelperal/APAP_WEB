...
A partir del merge del PR que introduce este ADR, el step `check_alantyle` del job `lint` en `.github/workflows/ci.yml` deja de usar `continue-on-error: true` y se vuelve `required`. Cero violaciones ALAN00x en `docs/`, `openspec/specs/`, `openspec/changes/`, `README.md`, `AGENTS.md`, `DOCS.md` y `CONTRIBUTING.md` bloquea el merge.

El ADR d-42 documenta la primera generación del rollout con `continue-on-error: true`; este documento lo sustituye en favor del gate duro.

## Quick path

Si su PR acumula una violación ALAN00x, corra `python3 scripts/check_alantyle.py docs/ openspec/specs/ openspec/changes/ README.md AGENTS.md DOCS.md CONTRIBUTING.md` localmente antes de pushear. Las tres vías de fix son, en orden de preferencia:
...
- El step `check_alantyle` permanece en el scope declarado en el comentario del job `lint`: `docs/`, `openspec/specs/`, `openspec/changes/` más los cinco archivos raíz. Mover el scope es decisión editorial humana, no de este PR.

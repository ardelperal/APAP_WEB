...
## Datos y compatibilidad LocalBackend

| Superficie | Propósito | Evidencia |
|---|---|---|
| Tablas de dominio | Estado de negocio en PostgreSQL | `app/main.py::lifespan` + queries de cada slice |
| Tabla `usuarios_autorizados` | Allowlist y roles | `app/core/auth.py`, `app/core/ports/auth_port.py` |
| Tabla `web_only_feature_shadow` | Estado de reconciliación web ↔ legacy | `migration/shadow_state.py` |
| API FastAPI separada | Raw SQL, OAuth simulado, magic-link y buckets en memoria | `app/core/local_backend/app.py` |
...
- [ ] Si cambia la API separada, verifique que no la presenta como parte montada de `app.main`.

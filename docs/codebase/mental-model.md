...
| App FastAPI + Jinja2 + HTMX con acceso directo a PostgreSQL | [`app/main.py`](../../app/main.py), [`app/core/local_backend/db.py`](../../app/core/local_backend/db.py) |
...
- **Capas no se cruzan**: una route nunca ejecuta SQL directamente; dominio y application no dependen de `LocalPostgresExecutor` (§31, §33.4).
...
- [ ] Antes de saltarse una capa (route con SQL directo, application con `LocalPostgresExecutor`), cite la regla que lo prohíbe en el PR.

...
- **Self-host en producción**: el contenedor `app/core/local_backend/app.py` reemplaza a LocalBackend cuando `APAP_LOCAL_BACKEND=true`. El switch vive en [`app/core/local_backend_url.py`](../../app/core/local_backend_url.py) con la precedencia `APAP_INSFORGE_URL` > `APAP_LOCAL_BACKEND` > producción LocalBackend. La rama activa del esfuerzo es `feat/641-self-host-backend-coolify` (issue #641).
...
| LocalBackend (PostgreSQL + Auth + Storage) | Backend de datos y auth gestionado; usado como fallback de desarrollo local cuando `APAP_LOCAL_BACKEND` está unset | [`app/core/local_backend.py`](../../app/core/local_backend.py) + variables `APAP_INSFORGE_URL`, `APAP_INSFORGE_SERVICE_KEY` | Si no responde, las requests devuelven 5xx; el seed bootstrap falla si las tablas no existen |
...
| `APAP_INSFORGE_URL` | URL del backend LocalBackend (operador override; gana sobre `APAP_LOCAL_BACKEND`) | unset | [`app/core/local_backend_url.py`](../../app/core/local_backend_url.py) |
...
- [ ] Si modifica el contrato con LocalBackend, actualice [Arquitectura LocalBackend](../architecture/architecture-local-backend-stack.md) y la sección correspondiente en este radial en la misma sesión.

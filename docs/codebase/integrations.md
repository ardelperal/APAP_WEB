...
| Coolify (deploy) | Plataforma de despliegue del servicio `app.main:app`, Postgres y variables de entorno | MCP server Coolify + job `deploy` en [`ci.yml`](../../.github/workflows/ci.yml) | Sin `COOLIFY_WEBHOOK_URL` el job `deploy` se salta; no es merge blocker en pre-MVP |

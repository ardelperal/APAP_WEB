...
- **Cliente único hacia InsForge**: solo [`app/core/insforge.py`](../../app/core/insforge.py) habla con InsForge; nadie lo importa fuera de `adapters/`, `di/` y `app/main.py` (§33.4). En producción el contenedor self-hosted en Coolify actúa como backend; InsForge queda como fallback de desarrollo local.
- **Self-host en producción**: el contenedor `app/core/local_backend/app.py` reemplaza a InsForge cuando `APAP_LOCAL_BACKEND=true`. El switch vive en [`app/core/insforge_url.py`](../../app/core/insforge_url.py) con la precedencia `APAP_INSFORGE_URL` > `APAP_LOCAL_BACKEND` > producción InsForge. La rama activa del esfuerzo es `feat/641-self-host-backend-coolify` (issue #641).
...
| `APAP_LOCAL_BACKEND` | Truthy (`1|true|yes|on`) activa el contenedor `local_backend/app.py` como destino de `InsForgeClient`; en producción siempre `true` | unset (InsForge remoto) | [`app/core/insforge_url.py`](../../app/core/insforge_url.py) |
...
| `APAP_INSFORGE_URL` | URL del backend InsForge (operador override; gana sobre `APAP_LOCAL_BACKEND`) | unset | [`app/core/insforge_url.py`](../../app/core/insforge_url.py) |

...
- [x] 2.2 Add a Coolify webhook step to the `deploy` job: `curl -fsS -X POST "${{ secrets.COOLIFY_WEBHOOK_URL }}"` with `failure-level: error` (CD-01).
...
- [ ] 2.3 ~~Add an `insforge_create-deployment` step to the `deploy` job; the step calls the MCP tool with `projectId` and `sourceDirectory`; env vars are NOT passed in this call (CD-02).~~ **Marcada N/A el 2026-06-19.** Razón: APAP_WEB es un backend FastAPI + Jinja2 + Tailwind desplegado a Coolify como Docker container. InsForge solo actúa como BaaS (PostgreSQL/Auth/Storage); la app no se sirve desde InsForge. El `insforge_create-deployment` MCP tool está diseñado para frontends SPA servidos por InsForge (Vercel-backed), no para backends Docker. Además, el "MCP access" del runner asumido por `design.md` línea 23 no existe en GitHub Actions estándar. La capacidad reemplazante (bootstrap del schema InsForge en producción) se aborda en una issue nueva, no aquí.

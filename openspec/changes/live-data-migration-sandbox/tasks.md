...
- [x] 4.3 **VERIFICATION**: Initial apply run had no credentials and wrote a redacted BLOCKED artifact. Continuation after the operator added an untracked project `.env`: the existing `Settings` loader supplied non-empty URL/service-key values without exposing them. Live GET-only probe of a synthetic nonexistent key returned authenticated `404` and unauthenticated `404`; no returned URL was available, so no HEAD was sent. Evidence hash `b363ee26beb599c73db053cf121e99425ca4c419a6f3c390b1c1c787f6391bea`; `Verdict: BLOCKED`; endpoint/header remain unpinned because auth protection was not distinguishable. Focused/migration/full tests, ruff, check-rules, coverage, and build passed.
- [x] 4.3a **STRICT TDD CONTINUATION**: RED first added `.env`/Settings loader coverage and both safe 404-proof forms (strategy-level and returned-URL HEAD). RED: 2 failed / 9 passed; then 1 failed / 11 passed while triangulating returned-URL 404. GREEN: 12 passed. Production now loads credentials through `app.core.config.get_settings()` when the CLI uses its real environment; injected `env={}` remains the missing-credentials test seam. A protected `404` can PASS only when the unauthenticated comparison is `401`/`403`; equal authenticated/unauthenticated `404` remains BLOCKED.
- [x] Continuation commit: `feat(migration): load storage probe credentials through settings` (SHA persisted to Engram after commit).

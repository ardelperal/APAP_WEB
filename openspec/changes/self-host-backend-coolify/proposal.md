...
Traer el backend de APAP_WEB bajo nuestro control en Coolify, eliminando la dependencia del BaaS LocalBackend (instancia `c3uc9dk6.eu-central.local_backend.app`). Hoy el `verify-fallback-ready` gate falla con `web_to_legacy_check_only` porque el LocalBackend de dev está "No backend services available" — la migration bidireccional no es operacionalmente confiable mientras dependa de un servicio gestionado cuyo provisioning depende de un OAuth interactivo que no puedo ejecutar desatendido. Esta propuesta cierra ese bloqueador: reemplaza LocalBackend por una API propia en FastAPI servida por el mismo proceso de la app, sobre Postgres 16 (Coolify `coolify-db` ya provisionado) y MinIO (S3-compatible) para las fotos. Mantiene el `LocalBackendClient` como Port backend-agnostic — solo cambia la URL base cuando `APAP_LOCAL_BACKEND=true`. La hexagonal architecture del proyecto (regla §31) hace este cambio muy limpio: el dominio y la lógica de negocio no se enteran del swap.
...
- **M0 — Backend self-hosted viable**: imagen Docker multi-stage con la app + el API nuevo, `docker-compose.yml` con `app+postgres+minio` para dev local, `coolify.yaml` con metadata para provisionar el servicio en producción. `LocalBackendClient` se queda; cuando `APAP_LOCAL_BACKEND=true` apunta a `http://localhost:8000/api` (la nueva API).
- **M0 — Replicación de los 3 endpoints LocalBackend** (`POST /api/database/advance/rawsql`, `GET/POST /api/storage/buckets[/...]`, `POST /api/auth/oauth/google[/callback]`) en `app/core/local_backend/`. El cliente `LocalBackendClient` no se entera del swap — los tipos y formas del JSON son los mismos.
- **M0 — MinIO en Coolify** como S3-compatible para fotos. Bucket `apap-photos` se crea idempotentemente en el bootstrap (como hoy hace `ensure_private_bucket` con LocalBackend). El Port de storage es `app/core/ports/storage_port.py` y el adapter `app/core/adapters/storage/s3_storage_adapter.py` lo implementa — la app no cambia.
...
- **M1 — OAuth de Google se queda como opción secundaria**: el adapter `oauth_local_backend_adapter` no se toca. La tabla `usuarios_autorizados` permite ambos flows (password O `anadido_por` que indica el flow original). Migrar usuarios existentes de OAuth → classic es opcional (pueden seguir con Google).
- **M2 — verify-fallback-ready gate verde contra el nuevo backend**: el round-trip E2E corre contra `APAP_LOCAL_BACKEND=true` + Postgres local. La pieza `web_to_legacy_check_only` del gate corre contra el API nuevo en lugar de LocalBackend. El test E2E legacy_postgres sigue verde.
...
| **D-SELF-08** | El API nuevo vive en `app/core/local_backend/` con FastAPI router | Mismo proceso que la app principal, distinto módulo. La integración es un flag `APAP_LOCAL_BACKEND=true` que cambia la URL del `LocalBackendClient`. Si el flag es false, el comportamiento es idéntico al actual (LocalBackend remoto). |
...
## Self-host vs LocalBackend (comparación operacional)

| Aspecto | LocalBackend (hoy) | Coolify self-host (M0) |
|---|---|---|
| Provisioning del backend | Requiere login interactivo OAuth o panel web de LocalBackend (no automatizable) | `coolify up` desde el CLI o `docker-compose up` en local |
| DB PostgreSQL | Compartida, gestionada por el vendor | Dedicada, en nuestro Coolify |
| Storage de fotos | S3-compatible gestionado (LocalBackend) | MinIO (S3-compatible, self-hosted) |
| Latencia DB | Cross-cloud (eu-central) | Mismo host (Coolify VPS) |
| Costo mensual | Variable según tier de LocalBackend | Fijo (Coolify VPS ya pagado) |
| Backups | Depende del plan de LocalBackend | Cron + pg_dump en Coolify |
| OAuth de Google | Built-in (gestionado por LocalBackend) | Sigue vía `oauth_local_backend_adapter` (sin cambios) |
| Login email/password | NO soportado por LocalBackend directamente | Clásico email/password + magic link |
| Multi-tenant | Requiere separar apps en LocalBackend | Costo cero extra (otro schema) |
| Rollback | Depende de LocalBackend, soporte 24/7 de pago | Local: restaurar DB desde backup |
| Tiempo de provisioning del backend | Días (soporte de LocalBackend) | Minutos (terraform/CLI de Coolify) |

## Runtime boundary del backend self-host (cómo difiere de LocalBackend)

| Capa | LocalBackend (hoy) | Coolify self-host (M0) |
|---|---|---|
...
| Auth OAuth Google | `/api/auth/oauth/google[/callback]` (HTTP) | Idéntico — el adapter `oauth_local_backend_adapter` no se toca |
...
| Base path de la API nueva | `/api/*` (mismo prefijo que LocalBackend, para no tocar el cliente) | El `LocalBackendClient` actual no necesita saber del swap |
...
- [ ] `app/core/local_backend/api.py` con FastAPI router que replica los 3 endpoints que `LocalBackendClient` consume
- [ ] `LocalBackendClient` apunta a `http://localhost:8000/api` cuando `APAP_LOCAL_BACKEND=true`
...
- [ ] La foto se sube a MinIO (no a LocalBackend storage) y se sirve vía el mismo endpoint autenticado
...
- **`LocalBackendClient` como Port** — la app no se entera del swap

## Consecuencias operacionales

- El operador pasa de depender de LocalBackend a depender de su propio Coolify. Backups locales, logs locales, control total.
...
- El verify-fallback-ready gate cambia de "LocalBackend unreachable" a "self-host local reachable" — un check de cableado, no un test de carga.

## Riesgos y mitigación

| Riesgo | Mitigación |
|---|---|
| El `LocalBackendClient` asume HTTP con el shape de LocalBackend; el nuevo API no es 100% compatible | La hexagonal architecture (Port) significa que el shape de los endpoints es la superficie. Tests E2E del round-trip validan que el cliente sigue funcionando. Si algo cambia, se ajusta en un solo lugar (`app/core/local_backend/api.py`). |
...
| Las sesiones existentes en LocalBackend se pierden | El session secret es nuevo; los usuarios tienen que re-loguearse. Es un trade-off aceptable para evitar mantener un LocalBackend residual. |
...
Esta propuesta reemplaza el BaaS LocalBackend por un backend self-hosted en Coolify, manteniendo la hexagonal architecture del proyecto. El cambio se hace en 3 fases (M0 backend viable, M1 login clásico con magic link, M2 deployment en Coolify), cada una con su acceptance y sus tests. La app no se entera del swap — solo cambia la URL del `LocalBackendClient`. El verify-fallback-ready gate queda green al final, lo que es la prueba operacional de que el backend está bien provisionado.

...
- [ ] 3.1 RED: create `tests/test_entradas_routes.py` for auth guards, service delegation, 404/409/422 handling, redirects, and no route `client.execute_sql(...)`.
- [ ] 3.2 GREEN: create `app/modules/entradas/routes.py` for protected list/detail/new/create/edit/update/delete routes; translate duplicate errors to 409.
- [ ] 3.3 GREEN: add `app/templates/entradas/list.html`, `detail.html`, and `form.html` with Spanish copy and only minimal intake fields.
- [ ] 3.4 GREEN: wire router/nav in `app/main.py` and `app/templates/base.html`; keep unrelated modules untouched.
- [ ] 3.5 Verify slice: run `pytest`, `ruff check .`, `python -m build`, then `code-review-expert` before PR 3 promotion.

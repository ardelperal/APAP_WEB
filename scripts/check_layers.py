...
import a web framework or a storage vendor (``fastapi``, ``local_backend``,
...
ALLOWED_IMPORTS: dict[str, frozenset[str]] = {
    ...
    # LocalBackend client itself). May depend on the port Protocols it
...
#: Path depth at which ``parts[1]`` exists (a flat ``app/<file>.py`` or a
#: one-segment subpackage like ``app/modules/`` / ``app/core/``).
DEPTH_AT_SEGMENT_1 = 2

#: Path depth at which ``parts[2]`` exists (the layer name under
#: ``app/core/``, the vendor under ``app/core/adapters/``, or a
#: slice name under ``app/modules/``).
DEPTH_AT_SEGMENT_2 = 3

#: Path depth at which ``parts[3]`` exists (a slice under a layer, or a
#: vendor-segment file under ``app/core/adapters/<vendor>/``).
DEPTH_AT_SEGMENT_3 = 4

#: Path depth at which ``parts[4]`` exists (a file inside
#: ``app/core/<layer>/<slice>/``, or a leaf file under
#: ``app/core/adapters/<vendor>/``).
DEPTH_AT_SEGMENT_4 = 5

#: Dotted-module depth at which a cross-slice ``app.modules`` import
#: still goes through the target's public package root
#: (``app.modules.<slice>``). Imports deeper than this reach into the
#: other slice's internals and are flagged as ``slice-internals``.
CROSS_SLICE_PUBLIC_ROOT_DEPTH = 3
...
def classify_layer(rel_posix: str) -> str | None:
    ...
    ``app/core/data_access.py`` holds the ``SqlExecutor`` Protocol and the
    Protocol-level exception hierarchy, so it is a port.
    """
    parts = rel_posix.split("/")
    if not rel_posix.endswith(".py") or parts[0] != ROOT_PACKAGE:
        return None
    if len(parts) < DEPTH_AT_SEGMENT_1:
        return None

    segment_1 = parts[1]
    if segment_1 == "modules":
        # The whole ``app/modules/`` tree is delivery, regardless of depth.
        return "delivery"
    if segment_1 != "core":
        return None

    return _classify_app_core_layer(parts)


def _classify_app_core_layer(parts: list[str]) -> str | None:
    """Dispatch a path under ``app/core/`` to its layer.

    Layered modules (``app/core/<layer>/...``) classify by their
    second-segment name; legacy flat modules
    (``app/core/<file>.py``) classify by their stem.
    """
    if len(parts) >= DEPTH_AT_SEGMENT_3:
        return _classify_app_core_by_layer_segment(parts[2])
    if len(parts) == DEPTH_AT_SEGMENT_2 and parts[2] == "__init__.py":
        # The ``app.core`` package root itself is a namespace, not a layer.
        return None
    return _classify_legacy_flat_core(parts[2])
...
def _classify_legacy_flat_core(stem_with_py: str) -> str:
    stem = stem_with_py.removesuffix(".py")
...
def classify_slice(rel_posix: str, layer: str | None) -> str | None:
    ...
    if layer == "delivery":
        return _slice_from_modules(parts)
    if len(parts) < DEPTH_AT_SEGMENT_3 or parts[1] != "core":
        return None
    return _slice_from_app_core(rel_posix, parts, layer)


def _slice_from_modules(parts: list[str]) -> str | None:
    """Resolve ``app/modules/<slice>/...`` -> the slice name."""
    if len(parts) >= DEPTH_AT_SEGMENT_2 and parts[1] == "modules":
        return parts[2]
    return None


def _slice_from_app_core(
    rel_posix: str, parts: list[str], layer: str
) -> str | None:
    """Resolve a slice for a file under ``app/core/<layer>/...``."""
    del rel_posix  # kept for symmetry with other classifiers; unused today.
    stem = parts[-1].removesuffix(".py")

    # A layer's package root re-exports every slice it contains, so it
    # belongs to no single slice — except in ``domain``, where
    # ``app/core/domain/__init__.py`` itself holds the catalog exports.
    if (
        stem == "__init__"
        and layer != "domain"
...
def _slice_from_subpackage(parts: list[str], stem: str, layer: str) -> str | None:
    """Resolve a slice whose name comes from the directory under a layer."""
    if layer == "adapters":
        # ``app/core/adapters/<vendor>/<slice>_<vendor>_<role>.py`` —
        # split on the vendor segment, not on the first underscore, or
        # ``schema_bootstrap_local_backend_adapter`` reads as ``schema``.
        vendor = parts[3]
        marker = f"_{vendor}"
...
def _check_slice(
    ...
    if layer == "delivery" and target_layer == "delivery":
        ...
        if depth <= CROSS_SLICE_PUBLIC_ROOT_DEPTH:
            return None
...
BASELINE: Mapping[str, str] = {
    ...
    "app/core/auth.py -> app.core.adapters.local_backend.auth_local_backend_adapter [layer-direction]": _EPIC_420,

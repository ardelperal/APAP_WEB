"""Dev server runner with the InsForge lifespan disabled.
...
and ``ensure_domain_schema`` against InsForge. In environments where
InsForge is unreachable (CI sandbox, local dev box without a service
...
Data routes that actually need InsForge (e.g. /animales listing
animals) will 500 because the real InsForge client is not configured
...
async def _noop_lifespan(_app):
    """Skip the InsForge bootstrap; yield nothing."""
    yield

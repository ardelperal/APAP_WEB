...
- [x] **T2 — Keep migration E2E on its provisioned PostgreSQL schema.** Remove obsolete credential-based backend selection and its incorrect `LocalPostgresExecutor(url, api_key)` call; test that operator settings cannot redirect the fixture. Commit with its test; record SHA and hosted CI.
...
- Current state: T1 merged via [PR #942](https://github.com/ardelperal/APAP_WEB/pull/942) as merge commit `4a2b560919f9e19a209b6b15bcb6d351e360462c`; its work-unit commit was `1fe84d6dc0518048a3357624e26e57bf77b874cd` (392 authored lines). T2 is implemented on `fix/930-migration-e2e-seam` in [PR #943](https://github.com/ardelperal/APAP_WEB/pull/943); its first hosted CI run passed, and merge remains pending.
- TDD: four expected RED cases for obsolete env/constructor behavior, two more for `.env`-only DSN isolation, and one for active production-executor construction. Focused tests GREEN. The three PostgreSQL E2E atoms passed against an ephemeral local PostgreSQL 16 container after the hermetic run skipped them.
- T2 local checks: `uv sync --frozen --extra dev`; `.venv/bin/python -m pytest -q tests/migration/test_e2e_legacy_postgres.py tests/migration/test_round_trip.py tests/migration/test_cli.py` (21 passed, 3 skipped without a database); `make verify PYTHON=.venv/bin/python` (4,649 passed, 19 skipped, 1 xfailed, 1 warning); `.venv/bin/python -m build --wheel --sdist` (wheel and sdist built); Ruff, diff check, branch-name and 400-line size gates passed. Then `APAP_TEST_POSTGRES_DSN=postgresql://postgres@127.0.0.1:<ephemeral-port>/apap_test .venv/bin/python -m pytest -q tests/migration/test_e2e_legacy_postgres.py` passed 5/5 with PostgreSQL 16; the container was removed. Hosted [`ci.yml` run 36056296885](https://github.com/ardelperal/APAP_WEB/actions/runs/36056296885) passed, including tests, integration, fallback, security, lint, typecheck, size, and build; full E2E was skipped under the PR event matrix. Merge remains pending. Rollback boundary: T2's two test-seam files and this task document.
...
- Native RDD assessment of the committed T2 slice: `medium`, `under_budget` (266 authored lines); no review is due for this candidate under the current delivery budget.
- Next step: rerun CI against the task-document evidence update, merge PR #943 when all current gates are green, and verify issue #930 closes.

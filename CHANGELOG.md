...
- `refactor`: drop LocalBackend error-handler hexagonal slice (port + adapter + DI + shim, closes #662):
    - `app/core/backend_error_handler.py`: deleted (103 lines).
    - `app/core/ports/backend_error_handler_port.py`: deleted (131 lines).
    - `app/core/adapters/local-backend/backend_error_handler_local_backend_adapter.py`: deleted.
    - `app/core/di/backend_error_handler_di.py`: deleted.
    - `tests/test_backend_error_handler.py` and `tests/test_slice_backend_error_handler.py`: deleted.
    - `app/core/ports/__init__.py`: drop `ErrorTranslationPort`, `ErrorUserResponse`, `TranslatableError` re-exports.
    - `app/core/adapters/local-backend/__init__.py`: drop `BackendErrorTranslation` re-export.
    - `app/main.py`: replace `register_backend_error_handler(app, get_backend_error_handler_port())` with a generic `@app.exception_handler(Exception)` that emits `log_safe("server.unhandled_error", ...)` and returns a non-leaking 502. The §32.P4 contract is preserved as a generic handler instead of an LocalBackend-specific binding.
...
- `chore(config)`: drop LocalBackend from the deployment surface (closes #654):
    - `Dockerfile`: drop `ENV APAP_INSFORGE_URL=http://localhost:7130` (the container no longer references LocalBackend).
    ...
    - `pyproject.toml`: drop `"local_backend"` from `keywords`; clean two stale comment references to `docs/architecture/architecture-local-backend-stack.md`.

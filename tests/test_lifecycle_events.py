...
The tests use an in-memory :class:`_FakeSqlExecutor` to exercise
``record_event`` end-to-end without touching the network, mirroring the
pattern in ``tests/test_catalogs.py`` (the Arc C migration target
pattern for replacing ``httpx.MockTransport``).
"""
...
def _empty_client() -> _FakeSqlExecutor:
    """Build a fake executor that records every call and returns ``[]``."""

"""Unit coverage for LocalBackend magic-link and bucket route contracts."""
...
from app.core.local_backend import storage
...
@pytest.fixture
def isolated_buckets(monkeypatch: pytest.MonkeyPatch) -> None:
    """Replace the module registry so bucket tests cannot leak state."""
    monkeypatch.setattr(
        storage,
        "_BUCKETS",
        {"existing": {"bucketName": "existing", "isPublic": False, "files": 0}},
    )


@pytest.mark.usefixtures("isolated_buckets")
@pytest.mark.parametrize("payload", [{}, {"bucketName": ""}])
def test_ensure_bucket_rejects_missing_or_empty_name(payload: dict[str, Any]) -> None:
    """Invalid names fail without mutating the registry."""
    with pytest.raises(ValueError, match="non-empty 'bucketName'"):
        storage.ensure_bucket(payload)
    assert list(storage._BUCKETS) == ["existing"]


@pytest.mark.usefixtures("isolated_buckets")
def test_ensure_bucket_returns_existing_bucket_unchanged() -> None:
    """Idempotency keeps the original visibility setting."""
    result = storage.ensure_bucket({"bucketName": "existing", "isPublic": True})
    assert result == {"bucketName": "existing", "isPublic": False, "files": 0}


@pytest.mark.usefixtures("isolated_buckets")
def test_ensure_bucket_creates_requested_bucket() -> None:
    """A new bucket is recorded with the requested visibility."""
    result = storage.ensure_bucket({"bucketName": "new", "isPublic": True})
    assert result == {"bucketName": "new", "isPublic": True, "files": 0}
    assert storage._BUCKETS["new"] == result

...
+ FakeSqlExecutor for SQL shape assertion (issue #732 Arc B).
"""
...
def _client_with_query_handler(
    ...
    """Fake whose response is chosen by inspecting the SQL.

    Used by the resumen-sanitario tests where the same fake must answer
    both the nchip-lookup SELECT and the resumen-row SELECT.
    """

    class _QueryAwareFake(_FakeSqlExecutor):
        def execute_sql(
            self, query: str, params: list[object] | None = None
        ) -> list[dict[str, object]]:
            self.calls.append((query, list(params or [])))
            return handler(query, list(params or []))

    fake = _QueryAwareFake()
    return fake, fake.calls

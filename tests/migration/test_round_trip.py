...
- **Rule 8 (no production mutation)**: ``FakeInsForge`` + injected
...
def test_round_trip_100_voluntarios_preserves_dni(round_trip_runner) -> None:
    ...
    # FakeInsForge stores the JSON-encoded string verbatim so the
...
def test_round_trip_detects_unsynced_edits_as_needs_review(
    ...
    # handler routed them to shadow. ``FakeInsForge`` does not
...
def test_round_trip_counts_preserved(round_trip_runner) -> None:
    ...
    table, so the assertion is on the FakeInsForge row count.
    """
    legacy_rows = [
        {
            "Voluntario": f"user-{i:03d}",

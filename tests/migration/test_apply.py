...
- **Rule 2 (DI)**: the Dysflow executor is injected via
  ...
  the ``FakeSqlExecutor`` instance built by ``apply_runner``. No global
  getters.
...
- **Rule 8 (no production mutation)**: never touches a real ``.accdb``
  or a real LocalBackend; the ``FakeSqlExecutor`` is hermetic.
...
def test_apply_legacy_to_web_inserts_new_rows(apply_runner) -> None:
    ...
    client: FakeSqlExecutor = captured["client"]
...
def test_apply_legacy_to_web_skips_existing_rows_by_natural_key(apply_runner) -> None:
    ...
    second = apply_runner(
        legacy_rows=legacy,
        client=first["client"],  # reuse the seeded FakeSqlExecutor
    )

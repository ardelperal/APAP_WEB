...
- **Rule 8 (no production mutation)**: ``web_client`` is a
  ``FakeLocalBackend`` and the Dysflow mock returns canned rows.
"""
...
def test_cli_apply_check_only_does_not_write(
    web_client: FakeLocalBackend, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_cli_reconcile_still_works(web_client: FakeLocalBackend) -> None:
...
def test_cli_reconcile_default_lists_both_directions() -> None:
    ...
    stream = io.StringIO()
    rc = main(["reconcile"], web_client=FakeLocalBackend(), shadow_state=shadow, stream=stream)  # type: ignore[arg-type]
    assert rc == 0

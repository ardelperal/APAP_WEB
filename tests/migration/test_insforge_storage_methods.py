...
The PR4b production methods on ``app.core.insforge.LocalPostgresExecutor``:
...
- Rule 2 (DI): the real ``LocalPostgresExecutor`` accepts a ``transport`` kwarg.
...
def _client(handler) -> LocalPostgresExecutor:
    """Build a real ``LocalPostgresExecutor`` whose transport is the test handler."""

...
def _client_recording() -> tuple[_FakeSqlExecutor, list[tuple[str, list[object]]]]:
    """Build a fake executor that records every SQL call."""
...
def test_ensure_catalogs_raises_when_create_table_fails() -> None:
    ...
    class _FailingFake:
        def execute_sql(self, sql, params=None):
            raise BackendError(500, {"error": "boom"})

        def close(self):
            pass

    with pytest.raises(BackendError):
        ensure_catalogs(_FailingFake())
...
def _client_returning(
    ...
    """Build a fake executor that returns the given rows."""
    fake = _FakeSqlExecutor()

...
def _seed_terapia_related_records(ep: _EphemeralPostgres) -> dict[str, str]:
    ...
    animal_id = str(uuid4())
    ep.execute(
        f"INSERT INTO animales (id, nchip, nombreanimal, especie, sexo, "
        f"fnacimiento, fecha_alta, activo) "
        f"VALUES ('{animal_id}', 'CHIP-TEST-SALUD', 'Luna', 'CANINA', 'H', "
        f"'2020-01-01', now(), true)"
    )

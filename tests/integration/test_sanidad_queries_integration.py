...
def test_build_batch_insert(
    ...
    assert str(inserted[0]["animal_id"]) == related["animal_id"]
...
def test_build_resumen_sanitario(ephemeral_postgres: _EphemeralPostgres) -> None:
    ...
    # Insert an actuacion so there's data to return. ``RETURNING id``
    # is appended so the conftest wrapper
    # (``tests/integration/conftest.py::execute``) does not raise on
    # the non-tuple result that an INSERT without RETURNING produces
    # under psycopg3's ClientCursor.
...
def test_build_get_animal_nchip(ephemeral_postgres: _EphemeralPostgres) -> None:
    ...
    # Insert an nchip for the animal. ``RETURNING id`` is appended so
    # the conftest wrapper does not raise on the non-tuple result that
    # an UPDATE without RETURNING produces under psycopg3's ClientCursor.
    nchip_value = "ABC123456"
    ephemeral_postgres.execute(
        f"UPDATE animales SET nchip = '{nchip_value}' "
        f"WHERE id = '{related['animal_id']}' "
        f"RETURNING id"
    )
